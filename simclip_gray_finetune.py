...
def simclip_gray_finetune(train_dataloader, test_dataloader, config, device):
    ...
    for epoch in range(finetune_epochs):
        ...
        for rgb_images,_, gray_images, _, label_y in tqdm(train_dataloader, desc=f"Epoch [{epoch+1}/{finetune_epochs}]"):
            ...
            gray_features_2 = projection_gray(gray_features)


            combined_features = torch.cat((rgb_features, rgb_features_2, gray_features, gray_features_2), dim=1)
        ...
        with torch.no_grad():
            for rgb_images, gray_images, labels in tqdm(test_dataloader, desc=f"Evaluating Epoch [{epoch+1}/{finetune_epochs}]"):
                ...
                gray_proj_features = projection_gray(gray_features)

                # Concatenate features
                combined_features = torch.cat((rgb_features, rgb_proj_features, gray_features, gray_proj_features), dim=1)
